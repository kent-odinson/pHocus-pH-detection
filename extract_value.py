...
def data(m):
    ...
    #XYZ_avg = list(map(float, cv2.mean(XYZ)[:3]))

    c = BGR_avg + l1 + HSV_avg + Lab_avg

    return c

dir_path = "pH Detection Using Smartphone/test images"   #image path
files = os.listdir(dir_path)

with open("C:/Users/ASUS/Documents/(())/Semester 7/Despro 2/pH Detection Using Smartphone/Test.csv", "a", newline='') as f: #filepath save CSV file
    writer = csv.writer(f)
